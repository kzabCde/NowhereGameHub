...
export default function GameCard({game,best,isFavorite,onFav,onDetail,onPlay}){
  ...
  return <div className='group surface-card p-4'>
    ...
    <div className='mt-3 flex flex-wrap gap-2 text-sm'>
      <button type='button' className='surface-control focus-frame px-2 py-1' onClick={()=>onFav(game.id)}>{isFavorite?'♥ โปรด':'♡ โปรด'}</button>
      <button type='button' className='surface-control focus-frame px-2 py-1' onClick={()=>onDetail(game)}>รายละเอียด</button>
