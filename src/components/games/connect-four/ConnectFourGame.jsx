...
export default function ConnectFourGame() {
  ...
  return <div className='space-y-4'><div className='glass p-3 flex gap-2 items-center'>ระดับ <DifficultySelector value={difficulty} onChange={setDifficulty} /><button className='glass px-3' onClick={reset}>Reset Game</button></div><p className='font-semibold'>{status}</p><ConnectFourBoard board={board} onColClick={move} winning={winning} isLocked={locked} /><div className='grid grid-cols-3 gap-2'><div className='glass p-2'>Player {scores.p}</div><div className='glass p-2'>Machine {scores.m}</div><div className='glass p-2'>Draw {scores.d}</div></div>{result && <div className='glass p-4 animate-in-pop'><p className={`text-lg font-bold ${result.tone}`}>{result.title}</p><p className='text-sm text-slate-200'>{result.detail}</p></div>}</div>;
}
